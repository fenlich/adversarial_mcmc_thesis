...
def get_device(device_str):
    if device_str == 'xpu':
        ...
        if torch.cuda.is_available():
            ...
            return torch.device('cpu')
    elif device_str == 'privateuseone:0':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ascii', action='store_true', help='ASCII-only attack or not')
    ...
    if args.device == 'privateuseone:0':
    ...
    if args.resume is not None:
        ...
        if args.checker is not None:
            ...
            df = {'original':[], 'perturbed':[],'True':[],'Pred_original':[],'Pred_perturbed':[],'success':[], 'Dist_char':[], 'Dist_token':[], 'similarity':[], 'd_lev':[], 'time':[]}
        if args.llm:
            succ_hard = 0
            df['success_hard'] = []
    ...
    for idx in tqdm(range(start,test_size)):
        ...
        if args.attack_name == 'our_method':
        ...
        df['d_lev'].append(RFLev.distance(orig_S, adv_example))    
        if args.checker is not None:
