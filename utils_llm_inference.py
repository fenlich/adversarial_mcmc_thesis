...
class Inference(object):
        ...
        if self.model_name not in ['chatgpt', 'gpt4']:
            ...
            if self.model_name == 'google/flan-t5-large':
                ...
                self.model = AutoModelForCausalLM.from_pretrained(
                        self.model_name,
                        torch_dtype=torch.float32, # Меняем на float32 для AMD
                ...
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name,
                    cache_dir = "/home/student/.cache/huggingface/hub",
                    torch_dtype=torch.float16,
                    trust_remote_code=True,
                    low_cpu_mem_usage=True, 
                    use_cache=False).to(self.device)
            ...
            elif self.model_name == 'h2oai/h2o-danube3-500m-chat':
                from transformers import AutoModelForCausalLM, AutoTokenizer
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name,
                    cache_dir = "/home/student/.cache/huggingface/hub",
            ...
            elif self.model_name == "HuggingFaceTB/SmolLM2-135M":
                from transformers import AutoModelForCausalLM, AutoTokenizer
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name,
                    cache_dir = "/home/student/.cache/huggingface/hub",
                ...
                self.tokenizer.pad_token = self.tokenizer.unk_token
                self.tokenizer.padding_side = 'left' #this is important! not right!
            elif self.model_name == "HuggingFaceTB/SmolLM2-360M-Instruct":
                from transformers import AutoModelForCausalLM, AutoTokenizer
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name,
                    cache_dir = "/home/student/.cache/huggingface/hub",
                ...
                self.tokenizer.pad_token = self.tokenizer.unk_token
                self.tokenizer.padding_side = 'left' #this is important! not right!   
            elif self.model_name == "Qwen/Qwen2.5-0.5B-Instruct":
                from transformers import AutoModelForCausalLM, AutoTokenizer
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name,
                    cache_dir = "/home/student/.cache/huggingface/hub",
    ...
    def __call__(self, SS, debug=True):
        preds = []
        for S in SS:
    ...
    def predict_by_local_inference(self, S, debug=True):
    ...
    def token_to_logit(self,input_ids,attention_mask = None):
        ...
        '''
        # if is_llama_family:
        # if 'llama' in self.model_name or 'vicuna-7b-v1.3' in self.model_name or 'TinyLlama' in self.model_name:
        if any(x in self.model_name.lower() for x in ['llama', 'tinyllama', 'vicuna', 'h2oai', 'smollm']):
    ...
    def emb_to_logit(self,inputs_embeds,attention_mask=None):    
        ...
        '''
        # if 'llama' in self.model_name or 'vicuna-7b-v1.3' in self.model_name or 'TinyLlama' in self.model_name:
        if any(x in self.model_name.lower() for x in ['llama', 'tinyllama', 'smollm', 'vicuna', 'h2oai']):
            out = self.model(inputs_embeds = inputs_embeds).logits
        
        elif 't5' in self.model_name:
            decoder_input_ids = torch.ones(inputs_embeds.shape[0], 1,torch_dtype=torch.int) \
    ...
    #     # elif 'llama' in self.model_name or 'vicuna-7b-v1.3' in self.model_name or 'TinyLlama' in self.model_name:
    #     elif any(x in self.model_name.lower() for x in ['llama', 'tinyllama', 'vicuna', 'h2oai', 'smollm']):        
    ...
    #         if 'smollm' in self.model_name.lower():
    ...
    #             filtered_logits[neg_id] = logits[neg_id]
    #             filtered_logits[pos_id] = logits[pos_id]
                
    #             result = filtered_logits.unsqueeze(0)
    #             return result
    #         else:
    #             return logits 
        ...
        with torch.no_grad():
            ...
            if 'smollm' in self.model_name.lower():  
                label_words = {
                    ...
                    'sst': ['positive', 'negative'],
                    'qnli': ['yes', 'no'],
                    'rte': ['yes', 'no']
                }
                
                target_words = label_words[self.args.dataset]
                ...
                for word in target_words:
                    ...
                    if token_id:
                        ...
                        target_ids.append(token_id[0] if token_id else None)
                
                filtered_logits = torch.full_like(logits, float('-inf'))
                for tid in target_ids:
                    if tid is not None and tid < len(filtered_logits):
                        filtered_logits[tid] = logits[tid]
