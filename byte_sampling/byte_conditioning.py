...
class ByteConditioning(object):
    ...
    def __init__(
        ...
        if tokenizer is not None:
            ...
            # Убираем device_map="auto" и ставим совместимый тип данных
            load_kwargs = (
                dict(torch_dtype=torch.float32, attn_implementation="eager")
                if load_kwargs is None
                else load_kwargs
            )
            # На всякий случай удаляем device_map, если он пришел снаружи
        ...
        # Жестко прописываем DirectML и переносим модель на видеокарту AMD
