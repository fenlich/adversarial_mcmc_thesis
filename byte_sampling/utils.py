...
def scatter_logsumexp(
    ...
    *,
    ...
    if not safe_all_neg_inf:
        ...
        s = torch.zeros(dim_size, dtype=m.dtype, device=m.device)
        # if torch.isnan(shifted_exp).any() or torch.isinf(shifted_exp).any():
        #     shifted_exp = torch.nan_to_num(shifted_exp, nan=-float('inf'), posinf=0, neginf=-float('inf'))
    
        # s = s.scatter_add_(0, index, shifted_exp)
        # 3. log-sum-exp
        return m + s.log()
