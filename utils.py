...
class llm_input_processor(object):
    def __init__(self,dataset,premise,model_name = 'llama'):
        ...
        if 'llama' in model_name or 'flan-t5-large' in model_name or 'TinyLlama' in model_name or 'h2oai' in model_name or 'qwen' in model_name.lower():
        ...
        if 'smollm' in model_name.lower():
            if dataset == 'sst2' or dataset == 'sst': 
                self.promptbegin = "Classify the following review as either 'positive' or 'negative': "
                self.promptend = ' The answer is:'     
...
def get_llm_embeddings(model, input_ids):
    if isinstance(model, LlamaForCausalLM)  or "smollm" in model.config.model_type.lower():
...
class MarginLossLLM():
    def __init__(self, target_class, tau):
        ...
        self.tau = tau
        # true_classes = истинные лейблы, которые должна предсказывать модель в случае, если все ок
    def __call__(self, logits, true_classes):
        if self.tau==0:
            # return -1 * (logits[:, true_classes]  - logits[:, self.target_class])
            return logits[:, true_classes]  - logits[:, self.target_class]
            # return logits[:, self.target_class] -  logits[:, true_classes]
        else:
            # return -1 * (logits[:, true_classes]  - logits[:, self.target_class] + self.tau).clamp(min=0) 
            return (logits[:, true_classes]  - logits[:, self.target_class] + self.tau).clamp(min=0) 

class MarginLossLLM_Charmer():
    def __init__(self, target_class, tau):
        self.target_class = target_class
        self.tau = tau
        # true_classes = истинные лейблы, которые должна предсказывать модель в случае, если все ок
...
def load_model(args):
    ...
    if args.llm:
        ...
        if args.attack_name in ['charmer', 'our_method']:
...
def rename_model(model):
    ...
    if type(model).__name__ == 'AlbertForSequenceClassification':
        ...
        pass
    elif type(model).__name__ == 'Qwen2ForSequenceClassification':
        model.embeddings = model.model.embed_tokens
        def qwen_encoder(**kwargs):
            outputs = model.model(**kwargs)
            return outputs.last_hidden_state[:, -1, :]
        model.encoder = qwen_encoder
        model.classifier_wrapped = model.score
    else:
        raise NotImplementedError 
    
    return model

    # elif type(model).__name__ == 'LlamaForSequenceClassification':
    #     model.embeddings = model.model.embed_tokens
    #     model.encoder = lambda **kwargs: model.model(**kwargs).last_hidden_state[:, -1, :]
    #     model.classifier_wrapped = model.score
...
def get_attacker(model_wrapper,args):
    if args.attack_name in ['charmer', 'full_bruteforce', 'bruteforce', 'bruteforce_random']:
        ...
        from our_method import MCMCAttack
        attack = MCMCAttack(model_wrapper,args)
    else:
        import textattack
